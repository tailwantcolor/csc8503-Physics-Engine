...
void DisplayPathfinding() {
	for (int i = 1; i < testNodes.size(); ++i) {
		Vector3 a = testNodes[i - 1.0f];
		Vector3 b = testNodes[i];
		

		Debug::DrawLine(a+Vector3(0,0.1,0), b+Vector3(0,0.1,0), Vector4(0, 1, 0, 0));
	}
}
...
void TestBehaviourTree() {
	...
	BehaviourAction* findKey = new BehaviourAction("Find Key", [&](float dt, BehaviourState state)->BehaviourState {
		...
		return state; // will be "ongoing" untill success
		}
	);
	...
	for (int i = 0; i < 5; ++i) {
		...
		std::cout << "We're going on an advcenture!\n";
...
void TestPushdownAutomata1(Window* w,float dt) {
	PushdownMachine machine(new IntroScreen());
...
int main() {
	...
	PushdownMachine MenuMachine(new MainMenu(g));
